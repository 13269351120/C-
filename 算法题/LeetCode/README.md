...
知识点：单调栈   
如果是正向考虑132，就要先找到一个递增的序列，然后凡是小的就更新1，大的就更新3，介于两者中间的就符合了要求。  
如果是反向考虑231，也是先找到一个递增序列，23，大的就更新3，再用原来的3来更新2，介于两者中间的就只更新2，小于2的就符合了要求。  
...
我一直在思考如何将这种方法按照合理的思路推理出来：我们正常思考，如何求一个矩形的最大呢？以一个点往两边扩展；有了一点单调栈的感觉后，我们可以将这个情况稍微简化一下：以一个点为锚点，如果右边还有递增，矩形面积还要扩大的，所以当前是不好确定的，要继续走着瞧，所以把这个高度要push到栈里去，保持一个单调递增的情况。如果是递减的，那么只要是刚刚出现高度比这个值大的右边界已经确定了，那么如何知道左边界呢？由于这个栈只保留单调递增的，左边界其实就是栈里的前一个值。  
我们回过头来看单调栈的作用：单调栈因为保证了单调性，这个性质一定帮我们做了一些事情的，有时候通过这个可以忽略一些值，比如那道取一个window里的最大值的题，有时候可以限制边界，比如这个求面积的限制住了左边界，这样就可以一路往前冲，直到遇到右边界，计算所有可以计算的值。   

85. Maximal Rectangle  
缘起：前一题的启发和链接   
知识点：单调栈  
同样在思考如何自然的联系到单调栈的思路中去：矩形嘛，又要确定边界，如果一下子二维的边界不好确定（因为需要4个值，上下左右），那么我们就一层一层来看，每一层都是一个前一题的情况。  
